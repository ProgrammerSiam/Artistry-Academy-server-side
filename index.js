...
const uri = `mongodb+srv://${process.env.MONGO_USER}:${process.env.MONGO_PASS}@cluster0.tlwiikw.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post("/jwt", (req, res) => {
      ...
      const token = jwt.sign(user, process.env.JWT_ACCESS_TOCKEN, {
        expiresIn: "50h",
      });
      res.send({ token });
    });
    ...
    app.patch("/coursefeepayment/:id", async (req, res) => {
      ...
      const result = await SelectedCoursecollection.updateOne(
        filter,
        updateDoc
      );
      console.log("result", result);
    ...
    app.patch("/availableseats/:classID", async (req, res) => {
      ...
      const updateDoc = {
        $set: {
          ...
          enrolled_Student: parseInt(singleClass.enrolled_Student) + 1,
        },
      };
